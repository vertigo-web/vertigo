...
pub struct WatchOpts {
    ...
    /// What ignore files to search for in watched directories (space-separated, .gitignore format)
    #[arg(long, default_value_t = {".gitignore".to_string()})]
    pub watch_ignore_lists: String,

    /// Additional globs to ignore in every watch path (space-separated, .gitignore format)
    #[arg(long, default_value_t = {"**/*.swp **/*.rs.bk".to_string()})]
