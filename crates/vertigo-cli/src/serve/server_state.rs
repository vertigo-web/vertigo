...
use vertigo::command::{response_browser, CommandBrowser};
        ...
        let mut inst = WasmInstance::new(
            ...
            Arc::new({
                ...
                move |command| match command {
                    CommandBrowser::FetchCacheGet => {
                        let response = response_browser::FetchCacheGet { data: None };

                        return response.to_json();
                    }
                    CommandBrowser::FetchExec { request, callback } => {
