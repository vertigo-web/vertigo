...
pub fn run(opts: NewOpts) -> Result<(), i32> {
    ...
    if let Ok(mut dir) = target_path.read_dir() {
        if dir.next().is_some() {
            log::error!("Destination dir ({}) is not empty", target_path.to_string_lossy());
            return Err(-1)
        }
    }
    ...
    if let Err(err) = fs::create_dir_all(&target_path) {
        log::error!("Can't create directory {}: {}", target_path.to_string_lossy(), err);
        return Err(-2)
    };

    // Paste files into it
    if let Err(err) = TEMPLATE.extract(Path::new(&opts.dest_dir).join(&opts.package_name)) {
        log::error!("Can't unpack vertigo stub to {}: {}", target_path.to_string_lossy(), err);
        return Err(-3)
    };
