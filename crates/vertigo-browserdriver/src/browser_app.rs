...
/// }
/// ```
pub fn start_browser_app<T: PartialEq + 'static>(driver: Driver, state: Computed<T>, render: fn(&Computed<T>) -> VDomElement) {
    ...
    let future = start_app(driver.clone(), state, render);
    driver.spawn(future);
}
