...
impl<K: JsJsonSerialize + JsJsonDeserialize, V: JsJsonSerialize + JsJsonDeserialize> JsJsonSerialize
    for MapItem<K, V>
{
...
impl<K: JsJsonSerialize + JsJsonDeserialize, V: JsJsonSerialize + JsJsonDeserialize>
    JsJsonDeserialize for MapItem<K, V>
{
    fn from_json(context: JsJsonContext, json: JsJson) -> Result<Self, JsJsonContext> {
        ...
        if let (Some(k), Some(v)) = (k, v) {
            let k = from_json::<K>(k)
                .map_err(|error| context.add(format!("key error => {}", error)))?;

            let v = from_json::<V>(v)
                .map_err(|error| context.add(format!("value error => {}", error)))?;

            return Ok(MapItem { key: k, value: v });
        }

        Err(context.add("Expected at this level of key k and v"))
    }
}
