...
use crate::{CallbackId, DropResource, WebsocketConnection, WebsocketMessage, command::WebsocketMessageFromBrowser, driver_module::api::{CallbackStore, api_browser_command}};
...
impl ApiWebsocket {
    ...
    pub fn websocket<F: Fn(WebsocketMessage) + 'static>(
        ...
        let (callback_id, drop) = self.store.register_with_id(move |callback_id, message: WebsocketMessageFromBrowser| {
